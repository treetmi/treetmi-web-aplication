...
import { Search, Plus, Edit2, Trash2, Link2, UploadCloud, AlertTriangle, Handshake } from "lucide-react"
...
export default function PartnersTab() {
  ...
  const fetchPartners = async () => {
    try {
      setIsLoading(true)
      const res = await fetch(`${API_BASE_URL}/partners/admin/list`)
  ...
  const handleFileChange = (
    ...
    reader.onload = (event) => {
      if (event.target?.result) {
        ...
        toast.success("Logo partner berhasil dipilih!")
      }
    }
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    if (!newLogoFile) {
      toast.error("Logo partner wajib diunggah!")
      return
    }
    ...
    try {
      ...
      formData.append("link_url", newLinkUrl.trim())
      formData.append("order", newOrder)
      formData.append("logo", newLogoFile)
      formData.append("isActive", "true")

      const res = await fetch(`${API_BASE_URL}/partners/admin/create`, {
        method: "POST",
        body: formData
      })
      ...
      if (data.success) {
        ...
        toast.error(data.message || "Gagal menambahkan partner.")
      }
    } catch (err) {
      console.error(err)
  ...
  const handleToggleActive = async (id: string, currentVal: boolean) => {
    try {
      const res = await fetch(`${API_BASE_URL}/partners/admin/toggle/${id}`, {
        method: "PUT"
      })
      const data = await res.json()
      ...
      if (data.success) {
        setPartners(prev =>
          prev.map(p => (p.id === id ? { ...p, isActive: !currentVal } : p))
        )
        toast.success(
          `Partner berhasil ${!currentVal ? "diaktifkan 🟢" : "dinonaktifkan 🔴"}`
        )
      } else {
        toast.error("Gagal memperbarui status partner.")
      }
    } catch (err) {
      console.error(err)
  ...
  const openEditModal = (partner: any) => {
    ...
    setEditLinkUrl(partner.link_url || "")
    setEditOrder(partner.order.toString())
    setEditLogoFile(null)
    setEditLogoPreview(`${API_BASE_URL.replace('/api/v1', '')}${partner.logo_url}`)
    setIsEditOpen(true)
  }
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/partners/admin/update/${selectedPartnerId}`, {
        method: "PUT",
        body: formData
      })
      const data = await res.json()
      ...
      if (data.success) {
        ...
        toast.error(data.message || "Gagal memperbarui partner.")
      }
    } catch (err) {
      console.error(err)
      toast.error("Gagal menghubungkan ke server API.")
  ...
  const openDeleteConfirm = (partner: any) => {
  ...
  const handleDeleteSubmit = async () => {
    try {
      setIsSubmitting(true)
      const res = await fetch(`${API_BASE_URL}/partners/admin/delete/${deletePartnerId}`, {
        method: "DELETE"
      })
      const data = await res.json()

      if (data.success) {
        toast.success(`Partner "${deletePartnerName}" berhasil dihapus.`)
        setIsDeleteOpen(false)
        fetchPartners()
      } else {
        toast.error(data.message || "Gagal menghapus partner.")
      }
    } catch (err) {
      console.error(err)
      toast.error("Gagal menghubungkan ke server API.")
  ...
  return (
    <div className="space-y-6 w-full text-[#1A1A19] dark:text-[#EAE9E4]">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center bg-white dark:bg-[#121211] border border-slate-200 dark:border-zinc-800 p-4 rounded-[2rem] shadow-sm w-full">
        ...
        <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
          <DialogTrigger
            render={
              <Button className="bg-[#FFD551] text-black border border-slate-200 dark:border-zinc-800 hover:bg-[#FFC83B] text-xs font-black italic h-11 px-5 rounded-2xl shadow-sm transition-all active:scale-95 w-full sm:w-auto shrink-0 cursor-pointer">
                <Plus className="mr-1 h-4 w-4" /> Tambah Partner Baru
              </Button>
            }
          />
          <DialogContent className="max-w-md bg-[#F8F7F3] rounded-[2rem] border border-slate-200 dark:border-zinc-800 p-8 shadow-2xl text-[#1A1A19]">
            <DialogHeader>
              <DialogTitle className="font-black text-2xl italic tracking-tighter uppercase">Tambah Partner Baru</DialogTitle>
              <DialogDescription className="text-xs font-bold italic text-slate-500">
                Tambahkan logo partner mitra, link redirect sponsor, serta urutan prioritas visual di footer website.
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleAddSubmit} className="space-y-4 pt-2">
              {/* Image upload selector */}
              <div className="flex flex-col items-center justify-center space-y-2 pb-2">
                <Label className="text-xs font-black italic text-slate-500">Logo Partner (SVG/PNG/WebP)</Label>
                <div className="relative group">
                  ...
                  <input
                    ...
                    title="Pilih Logo Partner (Max 5MB)"
                  />
                </div>
                <span className="text-[9px] font-bold text-slate-400 italic">Klik box untuk pilih file logo partner (Maksimal 5MB)</span>
              </div>

              <div className="space-y-1">
                <Label className="text-xs font-black italic text-slate-500">Nama Partner</Label>
                <Input
                  required
                  placeholder="Contoh: Asus ROG, Intel"
                  value={newName}
              ...
              <div className="space-y-1">
                <Label className="text-xs font-black italic text-slate-500">Link Partner (URL, Opsional)</Label>
                <Input
                  placeholder="Contoh: https://rog.asus.com"
                  value={newLinkUrl}
              ...
              <div className="space-y-1">
                <Label className="text-xs font-black italic text-slate-500">Urutan Tampil (Semakin kecil semakin depan)</Label>
                <Input
                  type="number"
                  placeholder="0"
                  value={newOrder}
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 w-full">
            {filteredPartners.map((partner) => (
              <Card key={partner.id} className="border border-slate-200 dark:border-zinc-800/80 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.01)] bg-white dark:bg-[#121211] overflow-hidden flex flex-col justify-between group hover:border-[#FFD551] transition-all duration-300">
                <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-4">
                  ...
                  <div className="w-full flex justify-between items-center pb-2 border-b border-[#F2F0EA] dark:border-zinc-800/50">
                    ...
                    <button
                      ...
                      title={partner.isActive ? "Nonaktifkan partner" : "Aktifkan partner"}
                    >
                      {partner.isActive ? (
                  ...
                  {/* Logo Container */}
                  <div className="w-20 h-20 rounded-[20px] border border-slate-100 dark:border-zinc-850 overflow-hidden bg-[#fafafa] dark:bg-zinc-950 shrink-0 flex items-center justify-center shadow-inner transition-all duration-300">
                    <img 
                      src={getFullLogoUrl(partner.logo_url)} 
                      alt={partner.name} 
                  ...
                  <div className="space-y-1">
                    <h4 className="text-xs font-black uppercase leading-tight line-clamp-1 italic text-black dark:text-white">{partner.name}</h4>
                    {partner.link_url ? (
                      <a 
                        href={partner.link_url} 
                        target="_blank" 
                        rel="noopener noreferrer" 
                        className="text-[9px] font-bold text-slate-400 dark:text-zinc-500 hover:text-[#FFD551] flex items-center gap-0.5 justify-center"
                      >
                        <Link2 className="h-2 w-2" /> Kunjungi Link
                      </a>
                    ) : (
                      <p className="text-[9px] font-bold text-slate-400 dark:text-zinc-500 italic">No external link</p>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2 w-full pt-2 border-t border-[#F2F0EA] dark:border-zinc-800/50">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => openDeleteConfirm(partner)}
          ...
          {filteredPartners.length === 0 && (
            <div className="text-center py-16 bg-white dark:bg-[#121211] border border-slate-200 dark:border-zinc-800 rounded-[2.5rem]">
              <Handshake className="h-10 w-10 text-slate-350 mx-auto mb-2" />
              <p className="text-slate-400 text-xs font-bold italic">Belum ada partner yang terdaftar.</p>
            </div>
          )}
        </>
      )}
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-md bg-[#F8F7F3] rounded-[2rem] border border-slate-200 dark:border-zinc-800 p-8 shadow-2xl text-[#1A1A19]">
          <DialogHeader>
            <DialogTitle className="font-black text-2xl italic tracking-tighter uppercase">Edit Partner</DialogTitle>
            <DialogDescription className="text-xs font-bold italic text-slate-500">
              Perbarui logo partner, nama, link, atau urutan tampil di footer.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleEditSubmit} className="space-y-4 pt-2">
            ...
            <div className="space-y-1">
              <Label className="text-xs font-black italic text-slate-500">Link Partner (URL)</Label>
              <Input
                value={editLinkUrl}
            ...
            <div className="space-y-1">
              <Label className="text-xs font-black italic text-slate-500">Urutan Tampil</Label>
              <Input
                type="number"
                value={editOrder}
        ...
        <DialogContent className="max-w-md bg-[#F8F7F3] rounded-[2rem] border border-slate-200 dark:border-zinc-800 p-8 shadow-2xl text-[#1A1A19]">
          <DialogHeader>
            <DialogTitle className="font-black text-2xl italic tracking-tighter uppercase flex items-center gap-2">
              <AlertTriangle className="h-6 w-6 text-red-500 animate-pulse shrink-0" /> Hapus Partner
            </DialogTitle>
            <DialogDescription className="text-xs font-bold italic text-slate-500">
              Apakah Anda yakin ingin menghapus partner ini secara permanen dari server?
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="py-4">
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center gap-4">
              <div className="w-16 h-16 rounded-[14px] border border-[#FFD551] overflow-hidden bg-white shrink-0 flex items-center justify-center">
                <img 
                  src={getFullLogoUrl(partners.find(p => p.id === deletePartnerId)?.logo_url || "")} 
                  alt="Target Delete" 
                  className="w-full h-full object-contain p-1" 
                />
              </div>
              <div className="flex flex-col min-w-0">
                <span className="text-xs font-black uppercase italic truncate">{deletePartnerName}</span>
                <span className="text-[9px] font-mono text-slate-400 truncate">{deletePartnerId}</span>
              </div>
            </div>
          </div>
