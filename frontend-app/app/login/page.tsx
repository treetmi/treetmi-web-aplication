...
export default function LoginPage() {
  ...
  const handleLoginRequestSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    if (!username) {
      setError(isIndo ? "Email atau Username wajib diisi!" : "Email or Username is required!")
      return
    }
    ...
    try {
      ...
      if (json.success && json.data) {
        ...
        setError(json.message || (isIndo ? "Gagal mengirim kode OTP masuk." : "Failed to send login OTP code."))
      }
    } catch (err: any) {
      console.error(err)
      setError(isIndo ? "Akun dengan Email/Username tersebut tidak ditemukan." : "Account with that Email/Username not found.")
    } finally {
      setIsLoading(false)
    }
  }
    ...
    try {
      ...
      if (json.success && json.token) {
        ...
        if (result?.error) {
          ...
          toast.success(isIndo ? "Selamat datang kembali di Treetmi Hub!" : "Welcome back to Treetmi Hub!")
      ...
      console.error(err)
      setError(isIndo ? "Gagal memverifikasi kode OTP masuk." : "Failed to verify login OTP code.")
    } finally {
      setIsLoading(false)
    }
  }
                    ...
                    <Card className="w-full border-2 border-slate-200/80 rounded-[2.5rem] shadow-xl bg-white dark:bg-[#121211] dark:border-zinc-800/80 overflow-hidden">
                      <CardHeader className="text-center space-y-2 pt-8">
                        ...
                        <CardDescription className="font-bold italic text-xs tracking-wider text-slate-500 dark:text-zinc-400">
                          {isIndo ? "Akses Dashboard Kreator Anda" : "Access Your Creator Dashboard"}
                        </CardDescription>
                      </CardHeader>
                      ...
                      <CardContent className="space-y-6 px-8 pb-8 pt-2">
                        <form onSubmit={handleLoginRequestSubmit} className="space-y-4">
                          ...
                          <div className="space-y-2">
                            <Label htmlFor="username" className="text-xs font-black italic text-slate-500 dark:text-zinc-400">
                              {isIndo ? "Alamat Email / Username Anda" : "Your Email Address / Username"}
                            </Label>
                            <div className="relative">
                              ...
                              <Input
                                id="username"
                                type="text"
                                placeholder={isIndo ? "contoh: budi@gmail.com atau budigamer" : "e.g., budi@gmail.com or budigamer"}
                                value={username}
                          ...
                          >
                            {isLoading ? (
                              ...
                              <>
                                {isIndo ? "Minta Kode OTP" : "Request OTP Code"} <ArrowRight className="h-4 w-4" />
                              </>
                            )}
                          </Button>
                        </form>
                        ...
                        <p className="text-center text-xs font-bold text-slate-500 dark:text-zinc-400 mt-4">
                          {isIndo ? "Belum memiliki akun?" : "Don't have an account?"}{" "}
                  ...
                  >
                    <Card className="w-full border-2 border-slate-200/80 rounded-[2.5rem] shadow-xl bg-white dark:bg-[#121211] dark:border-zinc-800/80 overflow-hidden">
                      <CardHeader className="text-center space-y-2 pt-8">
                        ...
                        <CardTitle className="text-4xl font-black italic tracking-tighter text-black dark:text-white">
                          {isIndo ? "Verifikasi Masuk" : "Login Verification"}
                        </CardTitle>
                      ...
                      <CardContent className="space-y-6 px-8 pb-8">
                        <form onSubmit={handleVerifyOTP} className="space-y-6">
                          ...
                          >
                            {isLoading ? (
                              ...
                              <>
                                {isIndo ? "Verifikasi & Masuk" : "Verify & Login"} <ArrowRight className="h-4 w-4" />
                              </>
                            )}
                          </Button>
                        </form>
                        ...
                        >
                          {isIndo ? "Kembali ke Form Masuk" : "Back to Login Form"}
                        </button>
                      </CardContent>
                    </Card>
                  </motion.div>
