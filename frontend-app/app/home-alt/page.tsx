...
export default function HomeAlt() {
  ...
  // Group logos for slider (5 per slide)
  const itemsPerSlide = 5
  const totalSlides = Math.ceil(paymentLogos.length / itemsPerSlide)
  ...
  return (
    <main className="min-h-screen bg-[#F8F7F3] selection:bg-[#FFD551] selection:text-black flex flex-col font-sans transition-colors duration-300 dark:bg-[#0A0A0A] dark:selection:bg-[#FFD551] dark:selection:text-black relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
            ...
            <div className="lg:col-span-6 space-y-6 text-center lg:text-left">
              ...
              >
                ...
                <span className="relative inline-block bg-[#FFD551] text-black px-6 py-2 mt-4 rounded-2xl shadow-[0_8px_20px_-4px_rgba(255,213,81,0.4)] dark:shadow-none font-display font-extrabold">
              ...
              >
                <Link href="/register">
                  <Button className="h-14 px-8 text-lg bg-[#FFD551] text-black font-display font-extrabold rounded-2xl shadow-sm hover:bg-[#FFC83B] hover:translate-y-[-1px] transition-all dark:shadow-none dark:border-white">
                    {t.common.getStarted} <ArrowRight className="ml-2 size-5" />
                  </Button>
                </Link>
                <Link href="/creators">
                  <Button variant="outline" className="h-14 px-8 text-lg rounded-2xl border-2 border-slate-200 bg-white text-slate-800 font-display font-extrabold shadow-sm hover:bg-slate-50 hover:translate-y-[-1px] transition-all dark:bg-[#1C1C1C] dark:text-white dark:border-zinc-800 dark:shadow-none">
            ...
            >
              <img 
                ...
                className="w-full max-w-[500px] h-auto object-contain hover:scale-[1.02] transition-transform duration-500 block"
              />
            </motion.div>
          </div>
        </div>
      ...
      <section className="py-16 bg-[#FAF9F6] border-y border-slate-200/60 dark:bg-[#0C0C0C] dark:border-zinc-900/60 z-10 relative">
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
            ...
            <div className="lg:col-span-5 flex flex-col justify-between p-7 bg-white dark:bg-zinc-900/80 border border-slate-200/80 dark:border-zinc-800/80 rounded-3xl shadow-xl space-y-4 text-left relative overflow-hidden backdrop-blur-md">
              <div className="space-y-4">
                
                {/* PREMIUM TAB MENU */}
                <div className="flex gap-2 p-1 bg-slate-100/80 dark:bg-zinc-950/80 rounded-2xl border border-slate-200/40 dark:border-zinc-800/40 relative">
                  <button
                    onClick={() => setSimType("donation")}
                    className={`flex-1 py-2.5 text-xs font-semibold rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1 ${
                      simType === "donation"
                        ? "bg-white dark:bg-zinc-800 text-amber-500 shadow-md border border-slate-200/40 dark:border-zinc-700/40 font-bold"
                        : "text-slate-500 hover:text-slate-800 dark:hover:text-zinc-200 font-medium"
                    }`}
                  >
                ...
                {simType === "donation" ? (
                  <div className="space-y-4 animate-fadeIn">
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-slate-500 tracking-wide">{t.home.sandbox.donatorName}</label>
                      <input 
                        type="text" 
                        value={simName}
                        onChange={(e) => setSimName(e.target.value)}
                        placeholder={t.home.sandbox.donatorNamePh}
                        className="w-full h-11 px-3.5 border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-950 dark:text-white rounded-xl text-sm font-medium transition-all outline-none focus:ring-1 focus:ring-[#FFD551] focus:border-[#FFD551]"
                      />
                    </div>

                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-slate-500 tracking-wide">{t.home.sandbox.amount}</label>
                      <input 
                        type="text" 
                        value={simAmount}
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-slate-500 tracking-wide">{t.home.sandbox.message}</label>
                      <textarea 
                        value={simMessage}
                        onChange={(e) => setSimMessage(e.target.value)}
                        placeholder={t.home.sandbox.messagePh}
                        rows={3}
                        className="w-full p-3.5 border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-950 dark:text-white rounded-xl text-sm font-medium transition-all outline-none resize-none focus:ring-1 focus:ring-[#FFD551] focus:border-[#FFD551]"
                      />
                    </div>
                  </div>
                ) : (
                  <div className="space-y-4 animate-fadeIn">
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-slate-500 tracking-wide">{t.home.sandbox.playerName}</label>
                      <input 
                        type="text" 
                        value={simName}
                        onChange={(e) => setSimName(e.target.value)}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="text-xs font-semibold text-slate-500 tracking-wide">{t.home.sandbox.gameId}</label>
                        <input 
                          type="text" 
                          value={simGameNick}
                      ...
                      <div className="space-y-1">
                        <label className="text-xs font-semibold text-slate-500 tracking-wide">{t.home.sandbox.gameServer}</label>
                        <input 
                          type="text" 
                          value={simGameId}
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-slate-500 tracking-wide">{t.home.sandbox.mabarFee}</label>
                      <input 
                        type="text" 
                        value={simAmount}
                        onChange={(e) => setSimAmount(e.target.value)}
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-slate-500 tracking-wide">{t.home.sandbox.mabarReq}</label>
                      <textarea 
                        value={simMessage}
                        onChange={(e) => setSimMessage(e.target.value)}
              ...
              <button
                ...
                className="w-full py-3 bg-[#FFD551] text-black font-bold text-sm rounded-xl shadow-md hover:bg-[#FFC83B] hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 transition-all flex items-center justify-center gap-1.5 cursor-pointer mt-4"
              >
                <Play className="size-4 fill-current animate-pulse" /> {t.home.sandbox.simulateBtn}
              </button>
            </div>

            {/* COLUMN RIGHT: LAYAR MONITOR OBS BESAR (7/12 cols) */}
            <div className="lg:col-span-7 flex flex-col justify-center">
              <div className="w-full aspect-[16/9.5] bg-[#121212] rounded-3xl p-3 border border-slate-200/80 dark:border-zinc-800/80 shadow-2xl relative flex flex-col overflow-hidden">
                <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 w-32 h-6 bg-slate-200 dark:bg-zinc-800 rounded-t-lg z-0" />
      ...
      <section className="pt-8 pb-16 bg-[#F8F7F3] dark:bg-[#0A0A0A] z-10 relative">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {/* Feature 1 */}
            <div className="p-6 rounded-3xl border border-slate-200 bg-white/60 backdrop-blur-md hover:-translate-y-2 hover:shadow-xl transition-all duration-300 dark:bg-zinc-900/40 dark:border-zinc-800 flex flex-col justify-between min-h-[220px] text-left relative overflow-hidden group">
              <div className="absolute top-0 left-0 w-1.5 h-full bg-[#FFD551]" />
              <div className="space-y-4">
                <div className="size-12 rounded-2xl bg-slate-900 text-[#FFD551] flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                  <Zap className="size-6" />
                </div>
                <div className="space-y-1.5">
                  <h3 className="text-base font-bold dark:text-[#EAE9E4]">{t.home.features.engine}</h3>
                  <p className="text-xs font-medium text-slate-500 dark:text-zinc-400">{t.home.features.engineDesc}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 rounded-3xl border border-slate-200 bg-white/60 backdrop-blur-md hover:-translate-y-2 hover:shadow-xl transition-all duration-300 dark:bg-zinc-900/40 dark:border-zinc-800 flex flex-col justify-between min-h-[220px] text-left relative overflow-hidden group">
              ...
              <div className="space-y-4">
                <div className="size-12 rounded-2xl bg-indigo-500/10 text-indigo-500 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                  <Gamepad2 className="size-6" />
                </div>
                <div className="space-y-1.5">
                  <h3 className="text-base font-bold dark:text-[#EAE9E4]">{t.home.features.mabar}</h3>
                  <p className="text-xs font-medium text-slate-500 dark:text-zinc-400">{t.home.features.mabarDesc}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 rounded-3xl border border-slate-200 bg-white/60 backdrop-blur-md hover:-translate-y-2 hover:shadow-xl transition-all duration-300 dark:bg-zinc-900/40 dark:border-zinc-800 flex flex-col justify-between min-h-[220px] text-left relative overflow-hidden group">
              ...
              <div className="space-y-4">
                <div className="size-12 rounded-2xl bg-green-500/10 text-green-500 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                  <ShieldCheck className="size-6" />
                </div>
                <div className="space-y-1.5">
                  <h3 className="text-base font-bold dark:text-[#EAE9E4]">{t.home.features.secure}</h3>
                  <p className="text-xs font-medium text-slate-500 dark:text-zinc-400">{t.home.features.secureDesc}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 rounded-3xl border border-slate-200 bg-white/60 backdrop-blur-md hover:-translate-y-2 hover:shadow-xl transition-all duration-300 dark:bg-zinc-900/40 dark:border-zinc-800 flex flex-col justify-between min-h-[220px] text-left relative overflow-hidden group">
              ...
              <div className="space-y-4">
                <div className="size-12 rounded-2xl bg-amber-500/10 text-amber-500 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                  <TrendingUp className="size-6" />
                </div>
                <div className="space-y-1.5">
                  <h3 className="text-base font-bold dark:text-[#EAE9E4]">{t.home.features.stats}</h3>
                  <p className="text-xs font-medium text-slate-500 dark:text-zinc-400">{t.home.features.statsDesc}</p>
                </div>
              </div>
            </div>
      ...
      {/* SUPPORTED PAYMENT CHANNEL SLIDER */}
      <section className="bg-white pt-8 pb-6 border-y border-[#EAE9E4] dark:bg-[#101010] dark:border-[#262626] relative overflow-hidden z-10">

        
        <div className="container mx-auto px-4 relative mt-2">
          <div className="max-w-4xl mx-auto overflow-hidden">
            ...
            >
              {Array.from({ length: totalSlides }).map((_, slideIdx) => (
                <div key={slideIdx} className="min-w-full grid grid-cols-5 gap-6 items-center justify-items-center px-4">
                  {paymentLogos.slice(slideIdx * itemsPerSlide, (slideIdx + 1) * itemsPerSlide).map((logo, i) => (
                    <motion.div 
                      key={i} 
                    ...
                    >
                      <img 
                        src={logo.url} 
                        alt={logo.name} 
                        className="h-10 md:h-12 w-auto object-contain transition-transform duration-500 group-hover:scale-105 filter grayscale hover:grayscale-0 dark:brightness-90 dark:contrast-125"
                      />
                    </motion.div>
                  ))}
                </div>
          ...
          <div className="flex justify-center gap-2 mt-6">
            {Array.from({ length: totalSlides }).map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-500 ${currentSlide === i ? "w-8 bg-black dark:bg-[#FFD551]" : "w-1.5 bg-muted-foreground/30"}`}
              />
            ))}
          </div>
        </div>
